...
void runState9()
{
	...
	* Setup Parameters
	*------------------------------------*/
	state9Type dt = 0.02;    //Sampling rate for the accelerometer in sec

	/*------------------------------------
	* Setup the UKF
	*------------------------------------*/
	UnscentedKalmanFilter<state9Type, NUM_STATE_VARS> UKF(true);
	...
	state9Type iPX = 0.25, iPV = 0.25, iPA = 1.25;

	Eigen::Matrix<state9Type, 3, 3> pBlock;
	...
	Eigen::Matrix<state9Type, 3, 3> physBlock;
	...
	// Process Noise: Q --------------------------------------------------
	Eigen::Matrix<state9Type, 3, 3> noiseBlock;
	...
	state9Type iRX = 1.25, iRV = 1.25, iRA = 0.25;

	Eigen::Matrix<state9Type, 3, 3> rBlock;
	...
	*------------------------------------*/
	Eigen::Matrix<state9Type, NUM_STATE_VARS, 1> mData, xOutData;
	Eigen::Matrix<state9Type, DOF, 1> ak, vk, xk, aklast, vklast, xklast;
	ak.setZero(DOF, 1);
	vk.setZero(DOF, 1);
	xk.setZero(DOF, 1);
