...
void runState3()
{
	...
	* Setup Parameters
	*------------------------------------*/
	state3Type dt = 0.02;      //Sampling rate for the accelerometer in sec
	...
	*------------------------------------*/
	...
	state3Type iPX = 0.25, iPV = 0.25, iPA = 1.25;

	Eigen::Matrix<state3Type, 3, 3> pBlock;
	...
	Eigen::Matrix<state3Type, 3, 3> physBlock;
	...
	// Process Noise: Q --------------------------------------------------
	Eigen::Matrix<state3Type, 3, 3> noiseBlock;
	...
	state3Type iRX = 1.25, iRV = 1.25, iRA = 0.25;

	Eigen::Matrix<state3Type, 3, 3> rBlock;
	...
	*------------------------------------*/
	Eigen::Matrix<state3Type, NUM_STATE_VARS, 1> mData, xOutData;
	Eigen::Matrix<state3Type, DOF, 1> ak, vk, xk, aklast, vklast, xklast;
	ak.setZero(DOF, 1);
	vk.setZero(DOF, 1);
	xk.setZero(DOF, 1);
